 ...
 * ```
 */ const importPersistentWebWorker = function(filename) {
...
const mathB = await importPersistentWebWorker(filename, WorkerOptions, {
    WorkerType: SharedWorker
});
const pool = new WebFunctionPool();
pool.allSettled(()=>{
...
pool.addTask(async ()=>{
    const math = await importPersistentWebWorker(filename, WorkerOptions, {
        WorkerType: SharedWorker
    });
