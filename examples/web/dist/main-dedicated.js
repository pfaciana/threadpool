 ...
 * ```
 */ const importTaskWebWorker = function(filename) {
...
const pool = new WebFunctionPool().allSettled(()=>{
    const completed = pool.status('completed', StatusType.RAW);
    for (const thread of completed){
        console.log(thread.message, thread.status.SUCCESS ? 'Success' : 'Error', thread.meta);
    }
...
const { add, getState } = await importWebWorker(filename);
const { fib, state } = await importTaskWebWorker(filename);
