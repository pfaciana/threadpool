...
export interface WorkerType {
	new(url: string | URL, options?: string | {});
}
 ...
 */
export const normalizeFilename = (f: string | URL): string => (f instanceof URL ? f.href : f)
 ...
 * ```
 */
export async function importWorkerProxy<T>(filename: string | URL, options: { executeImmediately: false, isPersistent?: false } & ImportOptions): Promise<DeferredPromisifyModule<T>>;
 ...
 * ```
 */
export async function importWorkerProxy<T>(filename: string | URL, options: { executeImmediately: true, isPersistent?: false } & ImportOptions): Promise<PromisifyModule<T>>;
 ...
 * ```
 */
export async function importWorkerProxy<T>(filename: string | URL, options: { executeImmediately: true, isPersistent: true } & ImportOptions): Promise<PromisifyModule<T & Terminable>>;
export async function importWorkerProxy<T>(filename: string | URL, options: ImportOptions): Promise<PromisifyModule<T> | PromisifyModule<T & Terminable> | DeferredPromisifyModule<T>> {
	const {
		isBrowser = false,
		isPersistent = false,
		...
		WorkerType = Worker as WorkerType,
		workerFile,
		workerOptions = {},
		workerPromise,
		messageOptions = { terminate: !isPersistent },
	} = options
	...
	return new Proxy(module, {
		get(target, prop) {
			...
			const runWorker = (...args: any[]): Promise<any> => {
				const message: WorkerMessage = typeof value === 'function' ? { filename, method: (prop === 'default' ? prop : value.name), args } : { filename, property: prop }
			...
			return executeImmediately ? runWorker : (...args: any[]) => () => runWorker(...args)
		},
	})
}
