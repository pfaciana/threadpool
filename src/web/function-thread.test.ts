...
import { WebFunctionThread, ThreadStatus, Status } from '@/index'

describe('WebFunctionThread', () => {
	let functionThread: WebFunctionThread
	const mockWorkerFn = vi.fn().mockImplementation(() => Promise.resolve())
	const mockMeta = { id: 1 }

	beforeEach(() => {
		mockWorkerFn.mockReset().mockImplementation(() => Promise.resolve())
		functionThread = new WebFunctionThread({ workerFn: mockWorkerFn, meta: mockMeta })
	})
	...
	describe('event handling', () => {
		it('should emit status change events', () => {
			...
			functionThread.addEventListener('status', ((event: CustomEvent) => {
				statusSpy(...event.detail)
			}) as EventListener)
		...
		it('should emit worker.init event on start', () => {
			const initSpy = vi.fn()
			functionThread.addEventListener('init', ((event: CustomEvent) => {
				initSpy(event.detail)
			}) as EventListener)
			functionThread.start()
	...
	describe('function execution', () => {
		it('should handle successful execution', async () => {
			...
			functionThread.addEventListener('message', ((event: CustomEvent) => {
				messageSpy(event.detail)
			}) as EventListener)
			functionThread.addEventListener('status', ((event: CustomEvent) => {
				statusSpy(...event.detail)
			}) as EventListener)
			functionThread.addEventListener('exit', ((event: CustomEvent) => {
				exitSpy(event.detail)
			}) as EventListener)
		...
		it('should handle execution error', async () => {
			...
			functionThread.addEventListener('error', ((event: CustomEvent) => {
				errorSpy(event.detail)
			}) as EventListener)
			functionThread.addEventListener('status', ((event: CustomEvent) => {
				statusSpy(...event.detail)
			}) as EventListener)
	...
	describe('edge cases', () => {
		it('should emit error event when promise catches an error', async () => {
			// This tests the specific catch block path that emits error
			...
			functionThread = new WebFunctionThread({ workerFn: catchableMockFn, meta: mockMeta })
		...
		it('should properly clean up event listeners', async () => {
			...
			// Using the once option for addEventListener (equivalent to once() in EventEmitter)
			functionThread.addEventListener('message', ((event: CustomEvent) => {
				oneShotSpy(event.detail)
