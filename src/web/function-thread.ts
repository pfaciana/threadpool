 ...
 * ```
 */
export class WebFunctionThread extends EventTarget {
	...
	then(onFulfilled: (value: any) => any): this {
		this.addEventListener('message', ((event: Event) => {
			onFulfilled((event as CustomEvent).detail)
		}) as EventListener)
		return this
	}
	 ...
	 */
		...
		this.addEventListener('error', ((event: Event) => {
			onRejected((event as CustomEvent).detail, 'error')
		}) as EventListener)
		this.addEventListener('messageerror', ((event: Event) => {
			onRejected((event as CustomEvent).detail, 'messageerror')
		}) as EventListener)
		return this
	}
	 ...
	 */
		...
		this.addEventListener('exit', (() => onFinally()) as EventListener)
		return this
	}
}
