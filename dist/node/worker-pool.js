...
import { startProfilingCpu, getTotalMemory, getLogicalCoreCount, getPhysicalCoreCount, isAnyThreadBelow } from 'system-resource-monitor';
import './worker.js';
import { WorkerThread } from './worker-thread.js';
import { TaskPool, StatusType } from '../pool-status.js';
import { exitEventSupported } from '../utils/exitEventSupported.js';
...
class WorkerPool extends EventEmitter {
    ...
    constructor(...options) {
        ...
        if (options.length > 2) {
            for (const [key, value] of Object.entries(options[2])) {
                this[key] = value;
            }
        }
    }
     ...
     * ```
     */
    async enableExitEventFallback(force = undefined) {
     ...
     * ```
     */
    addTask(workerData = undefined, meta = undefined) {
     ...
     */
    all(callback) {
        const wrappedCallback = (error, thread) => {
     ...
     */
    any(callback) {
        const wrappedCallback = (data, thread) => {
